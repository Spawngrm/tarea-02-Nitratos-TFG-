...
// Capa vectorial de polígonos en formato GeoJSON
$.getJSON("Datos/GIS/area.geojson", function (geodata) {
  var capa_area = L.geoJson(geodata, {
    style: function (feature) {
      return { color: "white", weight: 1, fillOpacity: 0 };  // Ajuste para polígonos
    },
    onEachFeature: function (feature, layer) {
      var popupText =
        "<strong>Nombre del distrito</strong>: " + feature.properties.distrito;
      layer.bindPopup(popupText);
    },
  }).addTo(mapa);

  control_capas.addOverlay(capa_area, "Zona de estudio");
});
...
// Capa de coropletas recarga acuifera
$.getJSON('Datos/GIS/recarga_acuifera.geojson', function (geojson) {
  var area_coropleta = L.choropleth(geojson, {
    valueProperty: 'id_recarga',
    ...
    onEachFeature: function (feature, layer) {
      layer.bindPopup('Nombre: ' + feature.properties.recarg_nac + '<br>' + 'Código Distrital: ' +
        feature.properties.id_codigo + '<br>' + 'Área en km2: ' + feature.properties.area.toLocaleString())
    }
  }).addTo(mapa);
  control_capas.addOverlay(area_coropleta, 'Área de recarga de las nacientes');
